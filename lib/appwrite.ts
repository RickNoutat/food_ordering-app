...
export const appwriteConfig = {
    ...
    platform: "com.icrea.foodordering",
...
export const getMenu = async ({ category, query }: GetMenuParams) => {
    try {
        ...
        let docs = menus.documents;
        if(query) {
            const q = query.toLowerCase();
            docs = docs.filter(doc => (doc.name as string).toLowerCase().includes(q));
        }
...
export const getPopularMenu = async (_: GetMenuParams) => {
    try {
        const menus = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.menuCollectionId,
            [Query.orderDesc('rating'), Query.limit(8)],
        )
